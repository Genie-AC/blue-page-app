...
export default function formatTitle(
  ...
  try {
    ...
    const fullTitle = buildFullTitle(baseTitle, page, city, bzn, acc, mod, locationInfo);
...
function formatDomainToBaseTitle(domain) {
  ...
  const locationPattern = new RegExp(`(\\w)(${ADJACENT_LOCATION_WORDS.join('|')})\\b`, 'gi');
  baseTitle = baseTitle.replace(locationPattern, '$1 $2');
  
  // Also handle the reverse case (location word followed by another word)
  const reverseLocationPattern = new RegExp(`\\b(${ADJACENT_LOCATION_WORDS.join('|')})(\\w)`, 'gi');
 ...
 */
function buildFullTitle(baseTitle, page, city, bzn, acc, mod, locationInfo = {}) {
  ...
  if (locationInfo.nearInTitle && (page || bzn || acc || mod)) {
    ...
    const basePart = baseTitle.replace(new RegExp(`\\s${locationInfo.locationWord}\\s*$`, 'i'), '');
    ...
    if (bzn && contentParam !== formatParam(bzn)) fullTitle += ` for ${formatParam(bzn)}`;
    if (acc && contentParam !== formatParam(acc)) fullTitle += ` for ${formatParam(acc)}`;
    if (mod && contentParam !== formatParam(mod)) fullTitle += ` ${formatParam(mod)}`;
...
function toTitleCase(str) {
  ...
  const alwaysUppercase = new RegExp(
    `(${ALWAYS_UPPERCASE.join('|')})`, 'g'
  );
